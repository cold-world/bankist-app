...
function calcDisplaySummary(account) {
  const income = account.movements.filter((item) => item > 0).reduce((acc, item) => acc + item, 0);
  labelSumIn.textContent = `${income}€`;
  const out = account.movements.filter((item) => item < 0).reduce((acc, item) => acc + item, 0);
...
btnLogin.addEventListener('click', (e) => userLogin(e));
function userLogin(e) {
  e.preventDefault();
  currentUser = accounts.find((item) => item.userName === inputLoginUsername.value);
...
btnLoan.addEventListener('click', (e) => loan(e));
function loan(e) {
  e.preventDefault();
  const amount = Number(inputLoanAmount.value);
  if (amount > 0 && currentUser.movements.some((item) => item >= amount * 0.1)) {
    currentUser.movements.push(amount);
    currentUser.movementsDates.push(new Date().toISOString());
  }
  updateUI(currentUser);
